resource "azurerm_network_interface" "nic" {
  ...
  name                = "${var.cluster_vm_name_prefix}-nic-${format("%02d", count.index + 1)}"
...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  name                = "${var.cluster_vm_name_prefix}-${format("%02d", count.index + 1)}"
  resource_group_name = var.resource_group_name
  location            = var.location
...
resource "azurerm_managed_disk" "data_disk" {
  ...
  name                 = "${var.cluster_vm_name_prefix}-data-${format("%02d", each.value.vm_index + 1)}-lun-${each.value.disk_lun}"
  location             = var.location
  resource_group_name  = var.resource_group_name
